...
class UploadVirusPanel extends Component {
    ...
    return (
      ...
      >
        <DialogTitle id="alert-dialog-slide-title">
          <FormattedMessage id="confirm_info_true" />
        </DialogTitle>
        <DialogActions>
          <Button
            onClick={() => {
  ...
  componentDidUpdate() {
    ...
    if (
      uploadPoiResult.status !== Status.IDLE &&
      ...
      if (uploadPoiResult.status === Status.SUCCESS) {
        ...
        if (uploadPoiResult.msg === VirusStatusActions.UPLOAD_POI_DATA) {
          uploadResultText = intl.formatMessage({
            id: "upload_virus_info_success"
          });
        } else if (uploadPoiResult.msg === VirusStatusActions.UPDATE_POI_DATA) {
          uploadResultText = intl.formatMessage({
            id: "update_virus_info_success"
          });
        } else if (uploadPoiResult.msg === VirusStatusActions.REPORT_POI_DATA) {
          uploadResultText = intl.formatMessage({
            id: "report_virus_info_success"
          });
        }
      } else if (uploadPoiResult.status === Status.FAILED) {
        severity = "error";
        if (uploadPoiResult.msg === undefined || uploadPoiResult.msg === "") {
          uploadResultText = intl.formatMessage({
            id: "upload_virus_info_fail_network"
          });
        } else {
            ...
            <Paper variant="outlined">
              <Grid direction="column" container>
                <Grid direction="row" container alignItems="center">
                  <Grid item xs>
                    <Typography className={classes.topTitleFont}>
                      <FormattedMessage id={topTitleText} />
                    </Typography>
                  </Grid>
                  ...
                  <Grid className={classes.gridContent}>
                    <FormControl>
                      <Field name="type" component={Select}>
                        <MenuItem value={"help"}>
                          <Typography className={classes.propsInputField}>
                            <FormattedMessage id="infection_help" />
                          </Typography>
                        </MenuItem>
                        <MenuItem value={"confirm"}>
                          <Typography className={classes.propsInputField}>
                            <FormattedMessage id="confirmed_infection" />
                          </Typography>
                        </MenuItem>
                        <MenuItem value={"cured"}>
                          <Typography className={classes.propsInputField}>
                            <FormattedMessage id="healing" />
                          </Typography>
                        </MenuItem>
                        <MenuItem value={"dead"}>
                          <Typography className={classes.propsInputField}>
                            <FormattedMessage id="death_from_infection" />
                          </Typography>
                        </MenuItem>
                      </Field>
                    </FormControl>
                ...
                {this.inputGridItem(
                  "condition_symptoms",
                  "weakness_fever_38_degrees",
                  false,
                  "symptom"
                )}
                {this.inputGridItem(
                  "places_visited_in_january",
                  "hebei_china_south_korea",
                  false,
                  "travel_history"
                )}
                {this.inputGridItem(
                ...
                <Button
                  ...
                  disabled={submitting}
                >
                  {/* <Typography className={classes.submitButtonFont}> */}
                ...
                <Grid direction="column" container alignItems="center">
                  <FormControlLabel
                    ...
                    label={
                      <Typography className={classes.titleFont}>
                        <FormattedMessage id="submit_detail_desc" />
                      </Typography>
                    }
                    labelPlacement="end"
                  />
